...
### API Key
A secret token used to authenticate requests to the Pictify API — a 64-character hex string passed as a Bearer token. Every key is live; there is no separate test-key type.
