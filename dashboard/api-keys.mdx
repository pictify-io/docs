...
- Full access; every render consumes plan credits
- Full API access
...
- (There is no separate test key type — use free-tier credits to experiment)
...
```bash
# .env.local
PICTIFY_API_KEY=YOUR_DEVELOPMENT_KEY
```
...
```bash
# staging.env
PICTIFY_API_KEY=YOUR_STAGING_KEY
```
...
PICTIFY_API_KEY=YOUR_PRODUCTION_KEY
```
...
Manage keys programmatically with the [API Keys API](/authentication):
