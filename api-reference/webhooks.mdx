...
<CodeGroup>
```bash cURL
curl https://api.pictify.io/webhook-subscriptions/wh_abc123 \
  -H "Authorization: Bearer $API_KEY"
```
...
<CodeGroup>
```bash cURL
curl -X PUT https://api.pictify.io/webhook-subscriptions/wh_abc123 \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
<CodeGroup>
```bash cURL
curl -X DELETE https://api.pictify.io/webhook-subscriptions/wh_abc123 \
  -H "Authorization: Bearer $API_KEY"
```
...
<Note>
Pause and resume are currently available via the dashboard only. API token support is coming soon.
</Note>

Temporarily pause a webhook subscription via the dashboard. Paused webhooks do not receive events.
...
Resume a paused webhook subscription via the dashboard.
