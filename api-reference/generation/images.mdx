...
<Note>
For dedicated template rendering with more options, use the [Template Render endpoint](/api-reference/templates#render) at `POST /templates/{uid}/render`.
</Note>
...
<CodeGroup>
```bash cURL
curl -X POST https://api.pictify.io/image/agent-screenshot \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
