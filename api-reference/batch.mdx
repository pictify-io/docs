...
The same endpoint accepts rows either inline or from a spreadsheet:

**Rows mode** — send `variableSets`, an array of variable objects, one per render:
...
`mappings` is `{ "csv_column": "templateVariable" }`. The CSV must be publicly fetchable — the server downloads it once when the batch starts.
