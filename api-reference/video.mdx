...
`format` is `"mp4"` (default) or `"gif"`. GIF output is palette-optimised and capped at 15fps and 720px wide, so files stay shareable — a GIF is the same template and the same render, encoded for places an MP4 cannot autoplay.
