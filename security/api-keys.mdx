...
An API key is a single 64-character hex string with full access to your team's resources. There are no key types, prefixes, or sandbox keys — every key is live, and every render it makes consumes your plan's monthly credits.
...
Each key includes:
- **Secret** - The key value itself (a 64-character hex string)
